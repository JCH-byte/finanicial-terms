...
export function QuizTab({ terms }) {
    ...
    const generateQuestion = () => {
        ...
        const randomIdx = Math.floor(Math.random() * terms.length);
        const answer = terms[randomIdx];
    ...
    if (!currentQuestion) return React.createElement('div', { className: 'p-10 text-center' }, '데이터 준비 중...');
    return React.createElement(
        'div',
        { className: 'flex flex-col h-screen pb-24 px-4 pt-8 bg-indigo-50' },
