...
export function MyNoteTab({ terms, bookmarks, toggleBookmark }) {
    const [selectedTerm, setSelectedTerm] = useState(null);
    const savedTerms = terms.filter((item) => bookmarks.includes(item.id));
    ...
    return React.createElement(
        ...
        savedTerms.length === 0
            ? React.createElement('div', { className: 'flex flex-col items-center justify-center py-20 mt-10' }, React.createElement('div', { className: 'bg-white p-6 rounded-full mb-4 shadow-sm border border-gray-100' }, React.createElement(Star, { className: 'text-gray-300', size: 40 })), React.createElement('p', { className: 'text-gray-500 font-bold text-lg' }, '아직 빈 노트입니다'), React.createElement('p', { className: 'text-gray-400 text-sm mt-1' }, '학습하다가 중요한 용어를 발견하면 별표하세요!'))
            : React.createElement(
                  'div',
                  ...
                  savedTerms.map((item) =>
                      React.createElement(
                          'div',
                          {
                              ...
                              className:
                                  'bg-white p-5 rounded-2xl shadow-sm border border-gray-100 flex justify-between items-center group hover:border-blue-200 transition-colors cursor-pointer active:scale-[0.98]',
                          },
                          React.createElement('div', null, React.createElement('span', { className: 'text-[10px] text-blue-500 font-bold bg-blue-50 px-1.5 py-0.5 rounded mb-1 inline-block' }, item.category), React.createElement('h3', { className: 'text-lg font-bold text-gray-900' }, item.term), React.createElement('p', { className: 'text-gray-400 text-sm mt-0.5 line-clamp-1' }, item.definition)),
                          React.createElement('button', { onClick: (e) => { e.stopPropagation(); toggleBookmark(item.id); }, className: 'p-2 text-yellow-400 hover:bg-yellow-50 rounded-full transition-colors' }, React.createElement(Star, { fill: 'currentColor', size: 24 })),
                      ),
                  ),
              ),
        ...
        selectedTerm &&
            React.createElement(
                ...
                React.createElement(
                    ...
                    React.createElement('div', { className: 'mb-6 mt-2' }, React.createElement('div', { className: 'flex space-x-2 mb-2' }, React.createElement('span', { className: 'text-blue-600 text-xs font-bold bg-blue-50 px-2 py-1 rounded-md' }, selectedTerm.category), React.createElement(DifficultyBadge, { level: selectedTerm.difficulty })), React.createElement('h2', { className: 'text-2xl font-bold text-gray-900' }, selectedTerm.term), selectedTerm.englishTerm && React.createElement('p', { className: 'text-gray-400 text-sm font-medium font-mono mt-1' }, selectedTerm.englishTerm)),
